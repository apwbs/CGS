...
def start(process_instance_id, message_id, slice_id, sender_address, output_folder, merged, function):
    """
    ...
    if function == "getApplicationForm":
        response = block_int.retrieveApplicationForm(message_id)
    elif function == "getGuaranteeConfirmation":
        response = block_int.retrieveGuaranteeConfirmation(message_id)
    elif function == "getLoanRequest":
        response = block_int.retrieveLoanRequest(message_id)
    elif function == "getGuaranteeClaim":
        response = block_int.retrieveGuaranteeClaim(message_id)
    elif function == "getRejectPayment":
        response = block_int.retrieveRejectPayment(message_id)
    elif function == "getAllowPayment":
        response = block_int.retrieveAllowPayment(message_id)
    elif function == "getGuaranteeClaimGuarantor":
        response = block_int.retrieveGuaranteeClaimGuarantor(message_id)
    else:
        print("The function you specified is not supported.")

    # response = block_int.retrieve_MessageIPFSLink(message_id)
    ciphertext_link = response[1]
...
if __name__ == '__main__':
    ...
    merged = {}
    start(process_instance_id, message_id, slice_id, sender_address, output_folder, merged, args.function)
