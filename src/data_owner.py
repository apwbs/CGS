...
def cipher_data(groupObj, maabe, api, process_instance_id, sender_name, input_files_path, policies_path, function, caseID):
    ...
    if caseID is None:
        ...
        metadata = {
            'sender': sender_address,
            'process_instance_id': int(process_instance_id),
            'message_id': int(caseID)
        }
        print(f'case id: {int(caseID)}')
    ...
    if function == "setApplicationForm":
        block_int.sendApplicationForm(sender_address, sender_private_key, message_id, hash_file)
    elif function == "setGuaranteeConfirmation":
        block_int.sendGuaranteeConfirmation(sender_address, sender_private_key, int(caseID), hash_file)
    elif function == "setLoanRequest":
        block_int.sendLoanRequest(sender_address, sender_private_key, int(caseID), hash_file)
    elif function == "setGuaranteeClaim":
        block_int.sendGuaranteeClaim(sender_address, sender_private_key, message_id, hash_file)
    elif function == "setRejectPayment":
        block_int.sendRejectPayment(sender_address, sender_private_key, int(caseID), hash_file)
    elif function == "setAllowPayment":
        block_int.sendAllowPayment(sender_address, sender_private_key, int(caseID), hash_file)
    elif function == "setGuaranteeClaimGuarantor":
        block_int.sendGuaranteeClaimGuarantor(sender_address, sender_private_key, int(caseID), hash_file)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--function', type=str, help='Smart Contract function to call.')
    parser.add_argument('-c', '--case_id', type=str, help='CaseID of this practice')
    ...
    cipher_data(groupObj, maabe, api, process_instance_id, args.sender_name, args.input, args.policies, args.function, args.case_id)
